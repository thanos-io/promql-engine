...
func (o *vectorOperator) execBinaryAnd(lhs, rhs model.StepVector, step *model.StepVector) error {
	...
	for i, sampleID := range lhs.SampleIDs {
		if jp := o.hcJoinBuckets[sampleID]; jp.ats == ts {
			step.AppendSampleWithHint(o.outputSeriesID(sampleID+1, 0), lhs.Samples[i], sampleHint)
		}
	}
	...
	for i, histogramID := range lhs.HistogramIDs {
		if jp := o.hcJoinBuckets[histogramID]; jp.ats == ts {
			step.AppendHistogramWithHint(o.outputSeriesID(histogramID+1, 0), lhs.Histograms[i], histogramHint)
		}
	}
	return nil
}
...
func (o *vectorOperator) execBinaryOr(lhs, rhs model.StepVector, step *model.StepVector) error {
	...
	for i, sampleID := range rhs.SampleIDs {
		if jp := o.lcJoinBuckets[sampleID]; jp.ats != ts {
			step.AppendSampleWithHint(o.outputSeriesID(0, sampleID+1), rhs.Samples[i], sampleHint)
		}
	}

	for i, histogramID := range rhs.HistogramIDs {
		if jp := o.lcJoinBuckets[histogramID]; jp.ats != ts {
			step.AppendHistogramWithHint(o.outputSeriesID(0, histogramID+1), rhs.Histograms[i], histogramHint)
		}
	}
...
func (o *vectorOperator) execBinaryArithmetic(ctx context.Context, lhs, rhs model.StepVector, step *model.StepVector) error {
	...
	for i, histogramID := range hcs.HistogramIDs {
		...
		case o.returnBool:
			h = nil
			if keep {
				step.AppendSampleWithHint(o.outputSeriesID(histogramID+1, jp.sid+1), 1.0, sampleHint)
			} else {
				step.AppendSampleWithHint(o.outputSeriesID(histogramID+1, jp.sid+1), 0.0, sampleHint)
			}
		case !keep:
			continue
		}
		...
		if h != nil {
			step.AppendHistogramWithHint(o.outputSeriesID(histogramID+1, jp.sid+1), h, histogramHint)
		}
	}
	...
	for i, sampleID := range hcs.SampleIDs {
		...
		if jp.histogramVal != nil {
			...
			step.AppendHistogramWithHint(o.outputSeriesID(sampleID+1, jp.sid+1), h, histogramHint)
		} else {
			...
			step.AppendSampleWithHint(o.outputSeriesID(sampleID+1, jp.sid+1), val, sampleHint)
		}
	}
	return nil
}
