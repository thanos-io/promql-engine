...
func TestDistributedAggregations(t *testing.T) {
	...
	queries := []struct {
		...
		{name: "label_replace before aggregation", query: `max(sum(label_replace(bar, "zone", "foo", "zone", "(.*)")))`},
		{name: "label_replace after aggregation", query: `max(label_replace(sum by (zone) (bar), "zone", "hardcoded-zone", "zone", ".*"))`},
