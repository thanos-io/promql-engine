...
func TestDistributedAggregations(t *testing.T) {
	...
	queries := []struct {
		...
		// increasing order of sample/histogram ids which are internal to each leaf querier, thus in some rare cases when order of output series is inconsistant,
		// the samples will differ from prometheus as root querier can't determine which sample would have occured first in sequential execution of prometheus,
		// this behaviour won't be an obstacle as limitk was proposed for an easier way to inspect labels in high cardinality metrics.
	}
